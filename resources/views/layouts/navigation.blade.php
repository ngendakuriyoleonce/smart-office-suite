        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <a href="{{ route('notifications.index') }}" class="relative mr-4 text-gray-500 hover:text-gray-700">
                    ...
                    @php $unread = \Illuminate\Support\Facades\Auth::user()->notifications()->whereNull('read_at')->count(); @endphp
                    @if ($unread > 0)
                        <span class="absolute -top-2 -right-2 inline-flex items-center justify-center min-w-[20px] h-5 px-1 text-xs font-bold text-white bg-red-600 rounded-full shadow ring-2 ring-white">{{ $unread }}</span>
                    @endif
                </a>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('notifications.index')" :active="request()->routeIs('notifications.*')">
                ...
                @php $unread = \Illuminate\Support\Facades\Auth::user()->notifications()->whereNull('read_at')->count(); @endphp
                @if ($unread > 0)
                    <span class="ml-1.5 inline-flex items-center justify-center min-w-[18px] h-4 px-1 text-[10px] font-bold text-white bg-red-600 rounded-full">({{ $unread }})</span>
                @endif
            </x-responsive-nav-link>
        </div>
